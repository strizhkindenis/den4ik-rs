...
fn layoutnf32(n: usize) -> Layout {
	Layout::array::<f32>(n).unwrap()
}

fn allocnf32(n: usize) -> NonNull<f32> {
	let layout = layoutnf32(n);
...
impl Drop for MatF32 {
	fn drop(&mut self) {
		let layout = layoutnf32(self.len());
...
impl MatF32 {
	...
	pub fn from_x(dims: Dims, x: f32) -> Self {
	...
	pub fn at(&self, idx: Index) -> &f32 {
		let i = self.dims.get_idx(idx);
		&self.as_slice()[i]	
	}
	
	pub fn at_mut(&mut self, idx: Index) -> &mut f32 {
		let i = self.dims.get_idx(idx);
		&mut self.as_slice_mut()[i]	
	}
	
	pub fn opp<F>(a: &Self, b: &Self, c: &mut Self, mut f: F)
	where F: FnMut(&f32, &f32, &mut f32)
	{
	...
	/// ```
	/// use den4ik_math::f32::{MatF32, Dims};
	/// let dims = Dims([2, 3]);
	/// let a = MatF32::from_esiter(dims, [
	///     1.0, 3.0, 2.0,
	///     4.0, 0.0, 1.0,
	/// ]);
	/// let b = MatF32::from_esiter(dims, [
	///     5.0, 2.0,  1.0,
	///     0.0, 3.0, -2.0,
	/// ]);
	/// let mut mc = MatF32::from_x(dims, 0.0);
	/// let c = MatF32::from_esiter(dims, [
	///     6.0, 5.0,  3.0,
	///     4.0, 3.0, -1.0,
	/// ]);
	/// MatF32::add(&a, &b, &mut mc);
	/// MatF32::assert_eq_eps(&mc, &c, 1e-6);
	/// ```
	...
	pub fn assert_eq_eps(a: &Self, b: &Self, eps: f32) {
